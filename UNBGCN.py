...
parser.add_argument('-D', '--dataset', type=str, default='ADNC')
...
parser.add_argument('-d', '--dropout', type=float, default=0.8, help='dropout rate')   
parser.add_argument('-f', '--filters', type=str, default='64,64,64', help='number of filters in each layer')      
parser.add_argument('-K', '--filter_scale', type=int, default=3, help='filter scale (receptive field size), must be > 0; 1 for GCN, >1 for ChebNet')  # 1
...
parser.add_argument('--seed', type=int, default=4111, help='random seed')
...
class GCN(nn.Module):
    ...
    def __init__(self,
        ...
        self.linear1 = nn.Linear(in_features-1, 17)
        self.linear2 = nn.Linear(17, in_features)
        ...
        for i in range(0, x.size(0)):
            ...
            for j in range(0, x.size(1)):
                ...
                for k in range(0, x.size(1)):
                    if k != j:
                        feat.extend(torch.mm(x[i][j].unsqueeze(0), x[i][k].unsqueeze(0).T))
                sim.append(torch.cat(feat, dim=0).unsqueeze(0))
            simialr.append(torch.cat(sim, dim=0).unsqueeze(0))
        return torch.cat(simialr, dim=0)

    def forward(self, data):
 
        similar = self.matrix(data[0])
        similar = self.linear1(similar)
        similar = self.linear2(similar)
        data[0] = data[0] * similar
...
if args.torch_geom:
    ...
    train_ids, test_ids = split_ids(rnd_state.permutation(len(dataset)), folds=n_folds)    #排列
...
def test(test_loader):
        #model.load_state_dict(torch.load('./graph.pth'))
        model.eval()
        start = time.time()
        ...
        for batch_idx, data in enumerate(test_loader):
            ...
            # if args.use_cont_node_attr:
            #     data[0] = norm_features(data[0])
            print(data[0].size(), data[1].size(), data[2].size(), data[3].size())
            output = model(data)
...
if __name__=="__main__":
    for epoch in range(args.epochs):
        train(loaders[0], epoch)  # no need to evaluate after each epoch
